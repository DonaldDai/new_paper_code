...
# [GCC 7.5.0]
# Embedded file name: /home/user/chenh0/TranSARMer_fine_tune/deep-molecular-optimization-main/trainer/seq2seq_trainer.py
# Compiled at: 2022-06-30 17:51:14
...
class Seq2SeqTrainer(BaseTrainer):
    ...
    def get_model(self, opt, vocab, device):
        if opt.starting_epoch == 1:
            model = Model.make_model(opt.num_layers, opt.layer_size, opt.cell_type, opt.embedding_layer_size, opt.dropout, opt.bidirectional, opt.bidirect_model, opt.attn_model, cfgd.DATA_DEFAULT['max_sequence_length'], vocab, mv.SMILESTokenizer(), self.LOG)
        else:
    ...
    def _initialize_optimizer(self, model, learning_rate):
        optimizer_encoder = torch.optim.Adam((model.network.encoder.parameters()), lr=learning_rate)
        optimizer_decoder = torch.optim.Adam((model.network.decoder.parameters()), lr=learning_rate)
        return (
    ...
    def validation_stat(self, dataloader, model, device, vocab):
        ...
        for _, batch in enumerate(ul.progress_bar(dataloader, total=(len(dataloader)))):
            ...
            total_tokens += ntokens
            total_loss += loss_b_sq.sum()
            predicted_seqs, predicted_nlls = model.greedy_sample(encoder_input, source_length, decoder_output, mask, device)
    ...
    def train(self, opt):
        ...
        for epoch in range(opt.starting_epoch, opt.starting_epoch + opt.num_epoch):
            ...
            model.network.encoder.train()
            model.network.decoder.train()
            loss_epoch_train = self.train_epoch(dataloader_train, model, optimizer_encoder, optimizer_decoder, opt.clip_gradient_norm, device)
            self.LOG.info('Training end')
            self.LOG.info('Validation start')
            model.network.encoder.eval()
            model.network.decoder.eval()
            with torch.no_grad():
                loss_epoch_validation, accuracy = self.validation_stat(dataloader_validation, model, device, vocab)
            self.LOG.info('Validation end')
            self.LOG.info('Train loss, Validation loss, accuracy: {}, {}, {}'.format(loss_epoch_train, loss_epoch_validation, accuracy))
...
# okay decompiling seq2seq_trainer.cpython-37.pyc
